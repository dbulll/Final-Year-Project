      ...
      <thead>
        <tr>
          <?php
            ...
            while($row = mysqli_fetch_array($sql))
            {
              echo '<th>'. $row['sprintName'] .'<br> '. date('d/m/Y', strtotime($row['sprintStartDate'])) .' - '. date('d/m/Y', strtotime($row['sprintEndDate'])) .'</th>';
            }
          ?>
        </tr>
      </thead>
      <tbody>
        ...
        <?php
          ...
          while ($rowcount > $counter)
            { echo '<td><div id="div1" ondrop="drop(event)" ondragover="allowDrop(event)" style="width:200px;height:70px;padding:10px;"></div></td>';
              $counter = $counter + 1;
            }
          $conn->close();
          ?>
